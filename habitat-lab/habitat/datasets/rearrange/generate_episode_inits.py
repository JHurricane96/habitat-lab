    ...
    with habitat.Env(config=config) as env:
        ...
        for i in tqdm(range(env.number_of_episodes)):
            ...
            for j in tqdm(range(10)):
                ...
                imgs = {}
                imgs["rgb"] = observation_to_image(
                    obs["robot_head_rgb"], "color"
                )
                imgs["depth"] = observation_to_image(
                    obs["robot_head_depth"].squeeze(), "depth", 1
                )
                imgs["semantic"] = observation_to_image(
                    obs["robot_head_semantic"], "semantic"
                )
                ...
                for key, img in imgs.items():
                    ...
                    img.save(str(fname))
                np.save(
                    str(imgs_out_dir / "semantic.npy"),
                    obs["robot_head_semantic"],
                )
